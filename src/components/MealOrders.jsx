...
import { getMealOrder, getMealOrders, getMealOrdersSocketUrl, validateMealOrder } from '../services/auth'
...
export default function MealOrders({ role }) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket = new WebSocket(getMealOrdersSocketUrl())
      socket.onopen = () => setStatus((value) => ({ ...value, socket: 'connected' }))
      socket.onmessage = ({ data }) => {
        const event = JSON.parse(data)
        if (event.type === 'MEAL_ORDER_CREATED' && tab === 'PENDING') setOrders((list) => list.some((item) => item.id === event.data.id) ? list : [event.data, ...list])
        if (event.type === 'MEAL_ORDER_VALIDATED') setOrders((list) => tab === 'PENDING' ? list.filter((item) => item.id !== event.data.id) : [event.data, ...list.filter((item) => item.id !== event.data.id)])
      }
  ...
  const OrderCard = ({ order }) => <article className={`meal-order ${order.status === 'VALIDATED' ? 'validated' : ''}`}><div className="order-service"><span>{order.meal_type === 'NOCHE' ? '☾' : '☀'}</span><div><small>Servicio</small><strong>{mealLabel(order)}</strong></div></div><div className="order-worker"><strong>{order.worker?.fullName || 'Trabajador'}</strong><span>{order.worker?.documentNumber || 'Documento no disponible'}</span></div><div className="order-meta"><span>{order.status === 'VALIDATED' ? 'Entregado' : 'Solicitado'}</span><strong>{displayTime(order.validated_at || order.claimed_at)}</strong></div><span className={`order-status ${order.status.toLowerCase()}`}>{order.status === 'PENDING' ? 'Pendiente' : 'Entregado'}</span>{role === 'COLLABORATOR' && order.status === 'PENDING' && <button onClick={() => setConfirming(order)}>Validar entrega</button>}</article>
  ...
  return <section className="orders-module"><header className="orders-heading"><div><p className="section-kicker">Atención en vivo</p><h1>Pedidos de comida</h1><p>Consulta y entrega los pedidos generados por los trabajadores.</p></div>{role === 'COLLABORATOR' && <span className={`socket-state ${status.socket}`}><i />{status.socket === 'connected' ? 'En vivo' : 'Reconectando'}</span>}</header>
    ...
    <div className="orders-tabs"><button className={tab === 'PENDING' ? 'active' : ''} onClick={() => setTab('PENDING')}>Pendientes</button><button className={tab === 'VALIDATED' ? 'active' : ''} onClick={() => setTab('VALIDATED')}>Entregados</button><span>{orders.length} pedidos</span></div>
    {status.loading ? <div className="list-loading"><span className="large-spinner" /> Cargando pedidos…</div> : orders.length ? <div className="orders-list">{orders.map((order) => <OrderCard order={order} key={order.id} />)}</div> : <div className="empty-orders"><span>✓</span><h2>{tab === 'PENDING' ? 'No hay pedidos pendientes' : 'No hay entregas registradas'}</h2><p>{tab === 'PENDING' ? 'Los nuevos pedidos aparecerán aquí automáticamente.' : 'Las entregas validadas aparecerán en este historial.'}</p></div>}
